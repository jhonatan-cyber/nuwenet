...
const required=['NUWENET_PORTAL_IP','NUWENET_PUBLIC_URL'];
const report={
  checked_at:new Date().toISOString(),config_file:source,mode:'read-only',
  config:{database:env.DB_DRIVER||'postgres',host:env.HOST||'127.0.0.1',port:env.PORT||'3000',presence:Object.fromEntries(required.map(k=>[k,present(k)]))},
