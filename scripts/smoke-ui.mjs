...
try {
  ...
  await page.getByRole('heading', { name: 'Hogar 100' }).waitFor();
  ...
  assert.equal(await page.locator('#account-trigger').evaluate(el=>el===document.activeElement),true,'Escape devuelve el foco al menú');
  await page.reload();
  await page.getByText('Titular actualizado', {exact:true}).waitFor();
  await openAccount('Configuraciones');
  ...
  await page.getByRole('button',{name:'Editar plan Hogar renovado',exact:true}).click();
  await page.keyboard.press('Escape');await planForm.waitFor({state:'hidden'});
  assert.equal(await page.getByRole('button',{name:'Editar plan Hogar renovado',exact:true}).evaluate(el=>el===document.activeElement),true,'El cierre devuelve el foco a Editar plan');
  ...
  assert.equal(await page.getByRole('button',{name:'Editar',exact:true}).evaluate(el=>el===document.activeElement),true);
