...
// API en apps/api/dist, como las demás herramientas de `tools/`.
...
try {
  if (comando === 'status') await status();
    ...
    console.error('Uso: bun tools/db/database.ts status|migrate|archive   (o bun run db:status / db:migrate / db:archive)');
    process.exitCode = 1;
  }
} catch (error) {
