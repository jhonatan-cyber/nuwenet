...
test('edificios: acceso aislado, actividad privada y formularios en el edificio seleccionado', async () => {
  ...
  try {
    ...
    assert.equal((await api(`state?building_id=${a}`)).enforcement.state,'error');
    ...
    cookie=ownerCookie;await api('buildings/central',{building_id:b,central_router_id:null},400);
