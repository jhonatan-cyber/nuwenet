...
test('D1/D4: paquete cifrado ilegible sin clave, redondo verificado y restauración', () => fixture(async ({ db, service, directory, pg }) => {
  ...
  const restore = spawnSync(process.execPath, ['tools/db/restore-backup.ts', source, destination], { encoding: 'utf8', windowsHide: true, env: process.env });
  ...
  try {
    await connection`UPDATE plans SET name='Changed after backup'`;
    const restoreArgs = ['--exit-on-error', '--clean', '--if-exists', '--schema', pg.schema, '--dbname', pg.env.DATABASE_URL || 'nuwenet', path.join(destination, 'nuwenet.dump')];
