...
const owner={id:uuidv7(),role:'superadmin',username:'owner'};
let customerQueue='nuwenet-department-1';
const sample=(down,up,id='*A',name=customerQueue)=>({id,name,target:'192.168.1.10/32',downloadBytes:down,uploadBytes:up,downloadRate:0,uploadRate:0});
async function fixture(run){
  ...
  try{
    ...
    await runAsSystem(async()=>{
      ...
      emitted.push((await management.createCustomer({apartment:'101',name:'Titular',plan_id:plan.id,ip:'192.168.1.10'})).portal_link);
      emitted.push((await management.createCustomer({apartment:'102',name:'Segundo',plan_id:plan.id,ip:'192.168.1.11'})).portal_link);
    });
    const [customerA,customerB]=await db.read(tx=>tx`SELECT id FROM customers ORDER BY apartment`);
    const [building]=await db.read(tx=>tx`SELECT id FROM buildings ORDER BY id LIMIT 1`);
    const routerId=uuidv7();customerQueue='nuwenet-department-'+customerA.id;
    await db.write(async tx=>{
      await tx`INSERT INTO routers(id,name,adapter,host,port,protocol,credentials,building_id) VALUES (${routerId},'Central','mikrotik-rest','192.168.1.1',443,'https','fixture',${building.id})`;
      await tx`UPDATE buildings SET central_router_id=${routerId} WHERE id=${building.id}`;
    });
    let usage=new UsageService(db,{});
    await run({get db(){return db;},get usage(){return usage;},management,routerId,customerA,customerB,portalTokens:emitted.map(e=>e.token),history:(id=customerA.id,month='2026-09')=>requestContext.run(owner,()=>usage.history(id,month)),restart:async()=>{await db.onModuleDestroy();db=new DatabaseService();await db.onModuleInit();usage=new UsageService(db,{});}});
  }finally{
...
test('consumo: primera lectura, reinicio por sentido, persistencia y duplicados',()=>fixture(async f=>{
  await f.usage.record(f.routerId,[sample(1_000_000,500_000)],'2026-09-10T12:00:00Z');
  assert.equal((await f.history()).totals.download_bytes,0);
  await f.usage.record(f.routerId,[sample(1_001_000,500_100)],'2026-09-10T12:01:00Z');
  await f.usage.record(f.routerId,[sample(200,500_200)],'2026-09-10T12:02:00Z');
  ...
  await f.restart();
  await Promise.all([f.usage.record(f.routerId,[sample(300,500_300)],'2026-09-10T12:03:00Z'),f.usage.record(f.routerId,[sample(300,500_300)],'2026-09-10T12:03:00Z')]);
  await f.usage.record(f.routerId,[sample(0,0)],'2026-09-10T12:01:00Z');
  ...
  assert.equal((await f.history(f.customerB.id)).totals.download_bytes,0);
}));
...
test('consumo: límites de mes en Bolivia y conservación exacta de bytes',()=>fixture(async f=>{
  ...
  await f.usage.record(f.routerId,[sample(1000,1000)],new Date(from).toISOString());
  await f.usage.record(f.routerId,[sample(1101,1051)],new Date(to).toISOString());
  assert.equal((await f.history(f.customerA.id,'2026-08')).totals.download_bytes,50);
  ...
  assert.equal(september.days.length,30);assert.equal((await f.history(f.customerA.id,'2028-02')).days.length,29);
  await assert.rejects(()=>f.history(f.customerA.id,'2026-13'),/Mes/);
}));

test('consumo: reemplazo, desaparición, huecos y cambio a cola compartida sin duplicar',()=>fixture(async f=>{
  await f.usage.record(f.routerId,[sample(100,50,'*L','nuwenet-192.168.1.10')],'2026-09-10T12:00:00Z');
  await f.usage.record(f.routerId,[sample(200,100,'*L','nuwenet-192.168.1.10')],'2026-09-10T12:01:00Z');
  await f.usage.record(f.routerId,[sample(1000,500),sample(900,400,'*L','nuwenet-192.168.1.10')],'2026-09-10T12:02:00Z');
  assert.equal((await f.history()).totals.download_bytes,100);
  await f.usage.record(f.routerId,[sample(1100,600)],'2026-09-10T12:03:00Z');
  await f.usage.record(f.routerId,[sample(9000,4000,'*NEW')],'2026-09-10T12:04:00Z');
  assert.equal((await f.history()).totals.download_bytes,200);
  await f.usage.record(f.routerId,[],'2026-09-10T12:05:00Z');
  await f.usage.record(f.routerId,[sample(15000,6000,'*NEW')],'2026-09-10T12:06:00Z');
  assert.equal((await f.history()).totals.download_bytes,200);
  await f.usage.record(f.routerId,[sample(16000,6500,'*NEW')],'2026-09-10T12:16:00Z');
...
test('consumo: contadores inválidos no producen reinicios falsos y servicio aislado',()=>fixture(async f=>{
  await f.usage.record(f.routerId,[sample(1000,500)],'2026-09-10T12:00:00Z');
  await f.usage.record(f.routerId,[sample(NaN,0)],'2026-09-10T12:01:00Z');
  assert.equal((await f.history()).coverage.router.status,'partial');
  await f.usage.record(f.routerId,[sample(1200,600)],'2026-09-10T12:02:00Z');
  assert.equal((await f.history()).totals.resets,0);assert.equal((await f.history()).totals.download_bytes,0);
  await assert.rejects(()=>f.usage.history(f.customerA.id,'2026-09'),/sesión/);
  await assert.rejects(()=>requestContext.run({id:uuidv7(),role:'admin'},()=>f.usage.history(f.customerA.id,'2026-09')),/acceso/);
  const [token1,token2]=f.portalTokens;
  assert.equal((await f.usage.history(null,'2026-09',token2)).coverage.first_sample,null);
  assert.equal((await f.usage.history(f.customerB.id,'2026-09',token1)).coverage.first_sample,'2026-09-10T12:00:00.000Z');
  await f.db.write(tx=>tx`UPDATE customers SET archived=1 WHERE id=${f.customerA.id}`);
...
test('consumo: error de router conserva acumulados y scheduler mide aunque fallen otras tareas',()=>fixture(async f=>{
  ...
  await f.db.write(tx=>tx`INSERT INTO users(id,username,password_hash,role,created_at) VALUES (${uuidv7()},'fixture','unused','superadmin',${new Date().toISOString()})`);
  let collected=0;
  const management={acquireTask:async()=> 'lock',releaseTask:async()=>{},syncLinkedDevices:async()=>{throw new Error('unrelated failure');},processQueue:async()=>{},settings:async()=>({overdue_minutes:0,auto_billing:false,backup_hours:0})};
