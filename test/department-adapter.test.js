...
test('MikroTik: un límite compartido, migración de cola individual y verificación',async()=>{
  ...
  try{
    ...
    await adapter.departmentSpeed(target,credentials,1,['192.168.1.11','192.168.1.10'],50,10);
    ...
    await adapter.departmentSpeed(target,credentials,1,['192.168.1.11'],100,20);assert.equal(queues.length,1);assert.equal(queues[0]['max-limit'],'20M/100M');
    rejectReadback=true;await assert.rejects(()=>adapter.departmentSpeed(target,credentials,1,['192.168.1.11'],100,20),/no confirmó/);
    rejectReadback=false;await adapter.departmentSpeed(target,credentials,1,[],100,20);assert.equal(queues.length,0);
