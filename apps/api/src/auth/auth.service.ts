...
export class AuthService {
  ...
  async setup(dto: SetupDto): Promise<SessionUser> {
    ...
    const rows = await this.database.write(async tx => {
      ...
      return tx<{ id: number }[]>`INSERT INTO users(username,password_hash,role,created_at) VALUES (${dto.username.trim()}, ${passwordHash}, 'superadmin', ${new Date().toISOString()}) RETURNING id`;
    });
  ...
  async login(dto: LoginDto): Promise<{ user: SessionUser; token: string }> {
    const [row] = await this.database.read(tx => tx<{ id: number; username: string; password_hash: string; role: string }[]>`SELECT * FROM users WHERE username=${dto.username.trim()}`);
    ...
    await this.database.write(tx => tx`INSERT INTO sessions(token_hash,user_id,created_at,expires_at) VALUES (${this.hashToken(token)}, ${row.id}, ${now.toISOString()}, ${expires})`);
  ...
  async validate(token: string | undefined): Promise<SessionUser | null> {
    ...
    const [row] = await this.database.read(tx => tx<{ user_id: number; expires_at: string; username: string; role: string }[]>`SELECT s.user_id, s.expires_at, u.username, u.role FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=${this.hashToken(token)} AND u.disabled=0`);
    if (!row) return null;
  ...
  async throttle(ip: string) {
    ...
    const allowed = await this.database.write(async tx => {
      ...
      await tx`INSERT INTO login_attempts(key,attempts,resets_at) VALUES (${ip},1,${reset}) ON CONFLICT(key) DO UPDATE SET attempts=login_attempts.attempts+1`;
      return true;
    });
  ...
  users() { return this.database.read(async tx => {
    ...
    for (const a of assignments as unknown as { user_id: number; building_id: number }[]) {
      if (!byUser.has(a.user_id)) byUser.set(a.user_id, []);
      byUser.get(a.user_id)!.push(a.building_id);
    }
    return (users as unknown as { id: number }[]).map(u => ({ ...u, building_ids: byUser.get(u.id) || [] }));
  }); }
  ...
  async updateUser(id: number, dto: UpdateUserDto) {
    await this.database.write(async tx => {
      ...
      if (user.role === 'admin' && !user.disabled && (dto.role !== 'admin' || dto.disabled)) {
        const [count] = await tx`SELECT COUNT(*) count FROM users WHERE role='admin' AND disabled=0`;
        if (Number(count.count) <= 1) throw new BadRequestException('Debe quedar al menos un administrador habilitado.');
      }
      await tx`UPDATE users SET role=${dto.role},disabled=${dto.disabled ? 1 : 0} WHERE id=${id}`;
