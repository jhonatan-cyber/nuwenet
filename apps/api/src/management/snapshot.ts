...
export async function readSnapshot(tx: TransactionSQL, settings: SettingsDto, query: StateQuery, database: string) {
  ...
  return {
    section:query.section || null,today,mode:networkStates.some(n=>n.router_id)?'mixed':'simulated',database,currency:settings.currency,settings,routers,
    ...
    plans:include('plans','overview','customers','network') ? await tx`SELECT p.*,(SELECT COUNT(*) FROM customers c WHERE c.plan_id=p.id AND c.archived=0 AND (${bid}::uuid IS NULL OR c.building_id=${bid}::uuid)) customer_count FROM plans p WHERE (${bid}::uuid IS NULL OR p.building_id=${bid}::uuid) ORDER BY id` : [],
