...
export class NetworkDesignService {
  constructor(private readonly db: DatabaseService, private readonly management: ManagementService) {}
  ...
  async apply(id: string, dto: ApplyNetworkDesignDto) {
    return this.db.write(async tx => {
      ...
      const data = await this.data(tx, id);
      if (data.building.central_router_id !== review.central_router_id) await this.management.setBuildingCentralInTransaction(tx, { building_id: id, central_router_id: review.central_router_id });
      const timestamp = new Date().toISOString();
