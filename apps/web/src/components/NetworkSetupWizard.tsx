...
import { ArrowLeft, ArrowRight, Cable, Plus, Save, Server, Trash2 } from 'lucide-react';
...
import type { RoutersContext } from '@/lib/routers-store';
...
export default function NetworkSetupWizard({ context, buildings, initialBuildingId, close, onOpenRouter }: { context: RoutersContext; buildings: { id: string; name: string }[]; initialBuildingId?: string | null; close: () => void; onOpenRouter?: (routerId: string) => void }) {
  ...
  return <PanelShell title="Configurar red del edificio" description="Organiza equipos y conexiones, comprueba la compatibilidad y revisa los cambios antes de aplicar."
    actions={<Button variant="outline" disabled={pending || dirty} onClick={close}><ArrowLeft />Volver a equipos</Button>}>
    ...
    {data && step === 1 && <div className="grid gap-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">Inventario del edificio</h2><Button variant="outline" disabled={pending} onClick={() => change({ ...design, nodes: [...design.nodes, { id: crypto.randomUUID(), name: '', role: 'switch' }] })}><Plus />Añadir equipo al diseño</Button></div>
      {!design.nodes.length && <Card><CardContent className="grid gap-3"><p className="text-sm text-muted-foreground">Añade los equipos o comienza con una estructura propuesta para los departamentos registrados. Confirma el cableado y los modelos reales.</p><Button className="w-fit" variant="outline" disabled={pending || data.customers.length > 247} onClick={template}>Crear estructura inicial</Button></CardContent></Card>}
    ...
    {data && step === 2 && <div className="grid gap-4"><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-semibold">Conexiones entre equipos</h2><Button variant="outline" disabled={pending || design.nodes.length < 2} onClick={() => change({ ...design, links: [...design.links, { from: design.nodes[0].id, to: design.nodes[1].id, from_port: '', to_port: '' }] })}><Cable />Añadir conexión</Button></div><p className="text-sm text-muted-foreground">Indica el cableado físico. Cada puerto puede usarse una vez. Esto documenta la conexión; no configura VLAN ni modifica los puertos.</p>
    ...
    {data && step === 4 && review && <div className="grid gap-4"><Card><CardHeader><h2 className="text-lg font-semibold">Cambios que se aplicarán</h2></CardHeader><CardContent className="grid gap-3"><ol className="list-decimal space-y-3 pl-5 text-sm">{review.steps.map(s => <li key={s}>{s}</li>)}</ol><div className="grid gap-2 border-t pt-4"><h3 className="text-sm font-semibold">Topología a publicar</h3><NetworkTopologyMap nodes={design.nodes} links={design.links} equipment={data.equipment.map(e => ({ id: e.id, status: e.status, compatibility: e.compatibility }))} customers={data.customers.map(c => ({ id: c.id, apartment: c.apartment }))} interactive onSelectRouter={onOpenRouter} />{design.links.map((l, i) => <p key={i} className="text-sm [overflow-wrap:anywhere]">{design.nodes.find(n => n.id === l.from)?.name} ({l.from_port}) → {design.nodes.find(n => n.id === l.to)?.name} ({l.to_port})</p>)}</div></CardContent></Card>
      ...
      <div className="flex flex-wrap gap-3"><Button variant="outline" disabled={pending} onClick={() => void run(inspectReview)}>Revisar de nuevo</Button><Button disabled={pending || !review.can_apply || dirty} onClick={() => void run(async () => { const result = await context.request(`building-networks/${building}/apply`, { revision: review.revision, fingerprint: review.fingerprint }); const updated = await context.request(`building-networks/${building}`); setData(updated); setDesign(updated.design); setReview(null); setDirty(false); setStep(0); setNotice(result.message); })}>Aplicar cambios revisados</Button></div>
    </div>}
    {data && <div className="flex flex-wrap items-center justify-between gap-3 border-t pt-5"><div className="flex flex-wrap gap-2"><Button variant="outline" disabled={pending || !dirty} onClick={() => void run(save)}><Save />Guardar borrador</Button><Button variant="ghost" disabled={pending} onClick={() => void run(async () => { const latest = await context.request(`building-networks/${building}`); setData(latest); setDesign(latest.design); setDirty(false); setReview(null); setStep(0); setNotice('Diseño guardado recargado.'); })}>Descartar y recargar</Button></div><div className="flex gap-2"><Button variant="outline" disabled={pending || step === 0} onClick={() => setStep(step - 1)}>Anterior</Button>{step < 4 && <Button disabled={pending} onClick={() => step === 3 ? void run(inspectReview) : setStep(step + 1)}>Continuar<ArrowRight /></Button>}</div></div>}
  </PanelShell>;
}
