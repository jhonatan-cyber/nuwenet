...
export default function AccountPanel() {
  ...
  return <Dialog open={state.open} onOpenChange={open => { if (!open) closeAccount(); }}>
    <DialogContent id="account-dialog" onCloseAutoFocus={event => {event.preventDefault();document.querySelector<HTMLButtonElement>('#account-trigger')?.focus();}}>
      <DialogHeader><DialogTitle>{settings ? 'Configuraciones' : 'Perfil'}</DialogTitle><DialogDescription>{settings ? 'Personaliza cómo se muestra tu panel.' : 'Administra tu cuenta y contraseña.'}</DialogDescription></DialogHeader>
      {settings ? <div className="grid gap-4">
