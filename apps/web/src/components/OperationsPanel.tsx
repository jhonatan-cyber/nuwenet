import { useEffect, useRef, useState, useSyncExternalStore, type ReactNode, type SubmitEvent } from 'react';
...
interface Building { id: number; name: string; address?: string | null; disabled?: number | boolean; central_router_id?: number | null }
interface RouterItem { id: number; name: string; adapter: string; disabled?: number | boolean; building_id?: number | null; status?: string }
interface UserItem { id: number; username: string; role: string; first_name?: string; last_name?: string; ci?: string; phone?: string; address?: string; disabled?: boolean; building_ids?: number[] }
...
function Shell({ title, description, actions, children, notice }: { title: string; description: string; actions?: ReactNode; children: ReactNode; notice: string }) {
  return <div className="shadcn-root grid gap-6 py-6">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div className="grid gap-2"><h1 className="text-2xl font-semibold tracking-tight">{title}</h1><p className="text-sm text-muted-foreground">{description}</p></div><div className="flex flex-wrap gap-2">{actions}</div></div>
...
function BuildingsView({ context }: { context: OperationsContext }) {
  ...
  if (!context.superadmin) return <Shell title="Edificios y accesos" description="Crea edificios, asigna administradores y define el equipo central de cada red." notice=""><Card><CardContent><p className="text-sm text-muted-foreground">Solo el super-admin gestiona edificios y accesos.</p></CardContent></Card></Shell>;
  const buildings = context.buildings;
  const centralName = (id?: number | null) => routers.find(r => r.id === id)?.name || 'Simulado';
  const options = (bid: number) => routers.filter(r => r.adapter === 'mikrotik-rest' && !r.disabled && (!r.building_id || r.building_id === bid));
  ...
  return <Shell title="Edificios y accesos" description="Crea edificios, asigna administradores y define el equipo central de cada red."
    ...
    <Card><CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead><tr><th className="p-3 text-left">Edificio</th><th className="p-3 text-left">Dirección</th><th className="p-3 text-left">Estado</th><th className="p-3 text-left">Equipo central</th><th className="p-3 text-left">Acciones</th></tr></thead><tbody>
      {buildings.map(b => <tr key={b.id} className="border-t"><td className="p-3"><strong>{b.name}</strong><br /><span className="text-muted-foreground">ID {b.id}</span></td><td className="p-3">{b.address || '—'}</td>
        <td className="p-3">{b.disabled ? <span className="font-medium text-destructive">Deshabilitado</span> : <span className="font-medium">Habilitado</span>}</td><td className="p-3">{centralName(b.central_router_id)}</td>
        <td className="p-3"><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size="sm" onClick={event => open({ type: 'edit', building: b }, event.currentTarget)}><Pencil aria-hidden="true" />Editar</Button><Button type="button" variant="outline" size="sm" onClick={event => open({ type: 'toggle', building: b }, event.currentTarget)}>{b.disabled ? 'Activar' : 'Desactivar'}</Button><Button type="button" variant="outline" size="sm" onClick={event => open({ type: 'remove', building: b }, event.currentTarget)}>Eliminar</Button></div></td></tr>)}
    ...
    {draft && <Dialog open onOpenChange={openState => { if (!openState) close(); }}>
      <DialogContent showCloseButton={!pending} onCloseAutoFocus={event => { event.preventDefault(); restore(); }} onEscapeKeyDown={event => { if (sending.current) event.preventDefault(); }} onPointerDownOutside={event => { if (sending.current) event.preventDefault(); }}>
        <DialogHeader><DialogTitle>{draft.type === 'new' ? 'Nuevo edificio' : draft.type === 'edit' ? `Editar edificio · ${current!.name}` : draft.type === 'toggle' ? (current!.disabled ? 'Activar edificio' : 'Deshabilitar edificio') : draft.type === 'remove' ? 'Eliminar edificio' : draft.type === 'central' ? `Equipo central · ${current!.name}` : `Dar acceso · ID ${current!.id}`}</DialogTitle>
        ...
        {draft.type === 'new' && <form className="grid gap-4" aria-busy={pending} onSubmit={event => { event.preventDefault(); const v = new FormData(event.currentTarget); void run(async () => { await context.request('buildings', { name: String(v.get('name') || '').trim(), address: String(v.get('address') || '').trim(), ...(v.get('admin_id') ? { admin_id: Number(v.get('admin_id')) } : {}) }); }, 'Edificio creado.'); }}>
          ...
          <FormError message={error} /><DialogFooter><Button type="button" variant="outline" onClick={close} disabled={pending}>Cancelar</Button><Button type="submit" disabled={pending}>{pending ? 'Guardando…' : 'Guardar'}</Button></DialogFooter>
        </form>}
        {draft.type === 'edit' && <form className="grid gap-4" aria-busy={pending} onSubmit={event => { event.preventDefault(); const v = new FormData(event.currentTarget); void run(async () => { await context.request('buildings/update', { building_id: current!.id, name: String(v.get('name') || '').trim(), address: String(v.get('address') || '').trim() }); }, 'Edificio actualizado.'); }}>
        ...
        {draft.type === 'central' && <form className="grid gap-4" aria-busy={pending} onSubmit={event => { event.preventDefault(); const v = new FormData(event.currentTarget); void run(async () => { await context.request('buildings/central', { building_id: current!.id, central_router_id: v.get('central_router_id') ? Number(v.get('central_router_id')) : null }); }, 'Equipo central actualizado.'); }}>
...
const numberFields = [
  ...
  { key: 'reminder_days', label: 'Anticipación del recordatorio, en días', min: 0, max: 30 },
...
function SettingsView({ context }: { context: OperationsContext }) {
  ...
  const [reminders, setReminders] = useState(Boolean(context.settings.reminders_enabled));
  const sending = useRef(false);
  const { capture, restore } = useDialogFocus();
  ...
  if (!context.superadmin) return <Shell title="Edificio y automatización" description="Define las reglas de operación y el equipo central que controla los departamentos." notice=""><Card><CardContent><p className="text-sm text-muted-foreground">Solo el super-admin configura la red y la automatización.</p></CardContent></Card></Shell>;
  async function submit(event: SubmitEvent<HTMLFormElement>) {
    ...
    body.auto_billing = autoBilling; body.reminders_enabled = reminders;
    sending.current = true; setPending(true); setError('');
    try {
  ...
  return <Shell title="Edificio y automatización" description="Define las reglas de operación y el equipo central que controla los departamentos."
    ...
    <Card><CardContent className="grid gap-4"><h2 className="text-lg font-semibold">Equipo central por edificio (solo super-admin)</h2><div className="overflow-x-auto"><table className="w-full text-sm"><thead><tr><th className="p-3 text-left">Edificio</th><th className="p-3 text-left">Central (MikroTik)</th><th className="p-3 text-left">Acción</th></tr></thead><tbody>
      {(context.buildings as Building[]).map(b => <tr key={b.id} className="border-t"><td className="p-3">{b.name}</td><td className="p-3">{routers.find(r => r.id === b.central_router_id)?.name || 'Simulado'}</td><td className="p-3"><Button type="button" variant="outline" size="sm" onClick={event => { capture(event.currentTarget); setCentral(b); }}>Cambiar central</Button></td></tr>)}
    ...
    <Card><CardContent>
      <form id="operations-form" className="grid gap-4" onSubmit={submit} aria-busy={pending}>
        ...
        <div className="flex items-center justify-between gap-3"><label htmlFor="settings-reminders" className="text-sm font-medium">Crear recordatorios internos de cobro</label><Switch id="settings-reminders" checked={reminders} onCheckedChange={setReminders} disabled={pending} /></div>
        <p className="text-sm text-muted-foreground">Los recordatorios quedan en Actividad. Las tareas se ejecutan mientras el servidor está encendido. Los respaldos incluyen la clave de los routers cuando existe.</p>
        <p id="settings-error" role="alert" className="text-sm text-destructive">{error}</p>
    ...
    {central && <Dialog open onOpenChange={openState => { if (!openState && !sending.current) setCentral(null); }}>
      <DialogContent showCloseButton={!pending} onCloseAutoFocus={event => { event.preventDefault(); restore(); }} onEscapeKeyDown={event => { if (sending.current) event.preventDefault(); }} onPointerDownOutside={event => { if (sending.current) event.preventDefault(); }}>
        <DialogHeader><DialogTitle>Central · {central.name}</DialogTitle><DialogDescription>Vacío = simulado.</DialogDescription></DialogHeader>
        <form className="grid gap-4" aria-busy={pending} onSubmit={event => { event.preventDefault(); if (sending.current) return; const v = new FormData(event.currentTarget); sending.current = true; setPending(true); setError(''); context.request('buildings/central', { building_id: central.id, central_router_id: v.get('central_router_id') ? Number(v.get('central_router_id')) : null }).then(() => { setCentral(null); setNotice('Equipo central actualizado.'); return context.refresh(); }).catch(err => setError(err instanceof Error ? err.message : 'No se pudo actualizar.')).finally(() => { sending.current = false; setPending(false); }); }}>
          <div className="grid gap-2"><label htmlFor="settings-central" className="text-sm font-medium">Router MikroTik</label><NativeSelect id="settings-central" name="central_router_id" defaultValue={central.central_router_id || ''} disabled={pending}><NativeSelectOption value="">Sin equipo · simulado</NativeSelectOption>{routers.filter(r => r.adapter === 'mikrotik-rest' && !r.disabled && r.building_id === central.id).map(r => <NativeSelectOption key={r.id} value={r.id}>{r.name}</NativeSelectOption>)}</NativeSelect></div>
          <FormError message={error} /><DialogFooter><Button type="button" variant="outline" onClick={() => setCentral(null)} disabled={pending}>Cancelar</Button><Button type="submit" disabled={pending}>{pending ? 'Guardando…' : 'Guardar'}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>}
  </Shell>;
...
function UsersView({ context }: { context: OperationsContext }) {
  ...
  if (!context.superadmin) return <Shell title="Usuarios y permisos" description="Super-admin: todo el sistema. Administrador: solo sus edificios. La red solo la configura el super-admin." notice=""><Card><CardContent><p className="text-sm text-muted-foreground">Solo el super-admin gestiona usuarios.</p></CardContent></Card></Shell>;
  const bname = (id: number) => context.buildings.find(b => b.id === id)?.name || `ID ${id}`;
  ...
  return <Shell title="Usuarios y permisos" description="Super-admin: todo el sistema. Administrador: solo sus edificios. La red solo la configura el super-admin."
    ...
    <Card><CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead><tr><th className="p-3 text-left">Administrador</th><th className="p-3 text-left">CI</th><th className="p-3 text-left">Contacto</th><th className="p-3 text-left">Edificios</th><th className="p-3 text-left">Estado</th><th className="p-3 text-left">Acción</th></tr></thead><tbody>
      {rows.map(u => <tr key={u.id} className="border-t"><td className="p-3"><strong>{fullName(u)}</strong><br /><span className="text-muted-foreground">{u.username}</span></td><td className="p-3">{u.ci || '—'}</td><td className="p-3">{u.phone || '—'}<br /><span className="text-muted-foreground">{u.address || ''}</span></td><td className="p-3">{(u.building_ids || []).map(bname).join(', ') || '—'}</td><td className="p-3">{u.disabled ? 'Deshabilitado' : 'Habilitado'}</td>
        <td className="p-3"><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" size="sm" onClick={event => open({ type: 'edit', user: u }, event.currentTarget)}>Editar</Button><Button type="button" variant="outline" size="sm" onClick={event => open({ type: 'buildings', user: u }, event.currentTarget)}>Edificios</Button><Button type="button" variant="outline" size="sm" onClick={event => open({ type: 'remove', user: u }, event.currentTarget)}>Eliminar</Button></div></td></tr>)}
    </tbody></table></CardContent></Card>
    ...
    {draft && <Dialog open onOpenChange={openState => { if (!openState) close(); }}>
      <DialogContent showCloseButton={!pending} onCloseAutoFocus={event => { event.preventDefault(); restore(); }} onEscapeKeyDown={event => { if (sending.current) event.preventDefault(); }} onPointerDownOutside={event => { if (sending.current) event.preventDefault(); }}>
        <DialogHeader><DialogTitle>{draft.type === 'new' ? 'Crear administrador' : draft.type === 'edit' ? `Permisos de ${current!.username}` : draft.type === 'buildings' ? `Edificios de ${current!.username}` : 'Eliminar usuario'}</DialogTitle>
        <DialogDescription>{draft.type === 'new' ? 'El administrador solo verá sus edificios asignados.' : draft.type === 'edit' ? 'Se cerrarán las sesiones de este usuario. Debe quedar un super-admin habilitado.' : draft.type === 'buildings' ? 'Asigna o retira edificios sin quitar los demás accesos.' : `¿Eliminar definitivamente a ${`${current!.first_name || ''} ${current!.last_name || ''}`.trim() || current!.username} (${current!.username})? Se quitarán sus accesos y sesiones. Esta acción no se puede deshacer.`}</DialogDescription></DialogHeader>
        {draft.type === 'new' && <form className="grid gap-4" aria-busy={pending} onSubmit={event => { event.preventDefault(); const v = new FormData(event.currentTarget); void run(async values => { await context.request('auth/users', { ci: String(values.get('ci') || '').trim(), first_name: String(values.get('first_name') || '').trim(), last_name: String(values.get('last_name') || '').trim(), address: String(values.get('address') || '').trim(), phone: String(values.get('phone') || '').trim(), username: String(values.get('username') || '').trim(), password: String(values.get('password') || ''), role: 'admin' }); }, 'Administrador creado.', v); }}>
        ...
        {draft.type === 'edit' && <form className="grid gap-4" aria-busy={pending} onSubmit={event => { event.preventDefault(); const v = new FormData(event.currentTarget); void run(async values => { await context.request(`auth/users/${current!.id}/update`, { role: String(values.get('role')), disabled: String(values.get('disabled')) === 'true' }); }, 'Permisos actualizados.', v); }}>
          <div className="grid gap-2"><label htmlFor="user-role" className="text-sm font-medium">Rol</label><NativeSelect id="user-role" name="role" defaultValue={current!.role} disabled={pending}><NativeSelectOption value="admin">Administrador de edificio</NativeSelectOption><NativeSelectOption value="superadmin">Super-admin (único)</NativeSelectOption></NativeSelect></div>
          <div className="grid gap-2"><label htmlFor="user-disabled" className="text-sm font-medium">Estado</label><NativeSelect id="user-disabled" name="disabled" defaultValue={current!.disabled ? 'true' : 'false'} disabled={pending}><NativeSelectOption value="false">Habilitado</NativeSelectOption><NativeSelectOption value="true">Deshabilitado</NativeSelectOption></NativeSelect></div>
          <FormError message={error} /><DialogFooter><Button type="button" variant="outline" onClick={close} disabled={pending}>Cancelar</Button><Button type="submit" disabled={pending}>{pending ? 'Guardando…' : 'Guardar'}</Button></DialogFooter>
        </form>}
        {draft.type === 'buildings' && <form className="grid gap-4" aria-busy={pending} onSubmit={event => { event.preventDefault(); const v = new FormData(event.currentTarget); void run(async values => { const wanted = context.buildings.filter(b => values.get(`bid_${b.id}`) === 'on').map(b => b.id); const currentIds = current!.building_ids || []; for (const bid of wanted.filter(b => !currentIds.includes(b))) await context.request('buildings/assign', { user_id: current!.id, building_id: bid }); for (const bid of currentIds.filter(b => !wanted.includes(b))) await context.request('buildings/unassign', { user_id: current!.id, building_id: bid }); }, 'Edificios actualizados.', v); }}>
        ...
        {draft.type === 'remove' && <div className="grid gap-4"><FormError message={error} /><DialogFooter><Button type="button" variant="outline" onClick={close} disabled={pending}>Cancelar</Button><Button type="button" disabled={pending} onClick={() => { void run(async () => { await context.request(`auth/users/${current!.id}/remove`, {}); }, 'Usuario eliminado.', new FormData()); }}>Eliminar</Button></DialogFooter></div>}
      </DialogContent>
    </Dialog>}
  </Shell>;
}
...
function BackupsView({ context }: { context: OperationsContext }) {
  ...
  return <Shell title="Respaldos" description="Copias verificadas de la base y la clave de los routers."
    ...
    <Card><CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead><tr><th className="p-3 text-left">Respaldo</th><th className="p-3 text-left">Fecha</th><th className="p-3 text-left">Motor</th><th className="p-3 text-left">Acción</th></tr></thead><tbody>
      {(list || []).map(b => <tr key={b.name} className="border-t"><td className="p-3">{b.name}</td><td className="p-3">{context.date(b.created_at)}</td><td className="p-3">{b.driver}</td>
        <td className="p-3"><Button type="button" variant="outline" size="sm" disabled={busy !== null} onClick={() => { if (busy) return; setBusy(b.name); context.request(`backups/${encodeURIComponent(b.name)}/verify`, {}).then(() => setNotice('Integridad del respaldo verificada.')).catch(err => setNotice(err instanceof Error ? err.message : 'No se pudo verificar.')).finally(() => setBusy(null)); }}>{busy === b.name ? 'Verificando…' : 'Verificar integridad'}</Button></td></tr>)}
    </tbody></table></CardContent></Card>
...
function AuditView({ context }: { context: OperationsContext }) {
  ...
  return <Shell title="Auditoría" description="Últimas 200 operaciones realizadas por usuarios autenticados."
    ...
    <Card><CardContent className="p-0 overflow-x-auto"><table className="w-full text-sm"><thead><tr><th className="p-3 text-left">Fecha</th><th className="p-3 text-left">Usuario</th><th className="p-3 text-left">Operación</th></tr></thead><tbody>
      {(list || []).map((a, i) => <tr key={i} className="border-t"><td className="p-3">{context.date(a.created_at)}</td><td className="p-3">{a.username}</td><td className="p-3">{a.action}</td></tr>)}
    </tbody></table></CardContent></Card>
  </Shell>;
}
