export interface Plan { id: string; name: string; down: number; up: number; price: number; customer_count: number; building_id: string; disabled: number }
export interface PlanInput { name: string; down: number; up: number; price: number; building_id?: string; id?: string; disabled?: number }
export interface PlansContext {
  ...
  save: (route: 'plans' | 'plans/update' | 'plans/toggle' | 'plans/delete', body: PlanInput | { id: string }) => Promise<unknown>;
